...
class ProductionValidator:
    ...
    def check_environment_variables(self) -> bool:
        ...
        self.log_result(
            "Environment Variables", True, "All required environment variables are set"
        )
        return True
    ...
    def check_docker_health(self) -> bool:
        ...
        try:
            ...
            character_ai_containers = [
                c
                for c in containers
                if "character-ai" in c.get("Names", "")
                or "character-ai" in c.get("Image", "")
            ]

            if not character_ai_containers:
                self.log_result(
                    "Docker Health", False, "No character-ai containers found running"
                )
                return False
            ...
            self.log_result("Docker Health", False, "Docker command timed out")
            return False
        except Exception as e:
            self.log_result(
                "Docker Health", False, f"Docker health check failed: {str(e)}"
            )
            return False
    ...
    def check_api_health(self) -> bool:
        ...
        try:
            ...
            if not health_data.get("status") == "healthy":
                self.log_result(
                    "API Health", False, f"API reports unhealthy status: {health_data}"
                )
                return False

            self.log_result(
                "API Health", True, "API health endpoint is responding correctly"
            )
            return True

        except requests.exceptions.RequestException as e:
            self.log_result("API Health", False, f"API health check failed: {str(e)}")
            return False
    ...
    def check_model_files(self) -> bool:
        ...
        required_files = ["llm/tinyllama-1.1b-q4_k_m.gguf"]
        ...
        self.log_result("Model Files", True, "All required model files are present")
        return True
    ...
    def check_disk_space(self) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                self.log_result(
                    "Disk Space", False, f"Failed to check disk space: {result.stderr}"
                )
                return False
            ...
            if len(lines) < 2:
                self.log_result(
                    "Disk Space", False, "Could not parse disk space information"
                )
                return False
            ...
            available = parts[3]
            self.log_result("Disk Space", True, f"Available disk space: {available}")
            return True

        except Exception as e:
            self.log_result("Disk Space", False, f"Disk space check failed: {str(e)}")
            return False
            ...
            try:
                if method == "GET":
                    ...
                    response = requests.request(
                        method, f"{self.base_url}{endpoint}", timeout=self.timeout
                    )
                ...
                self.log_result(
                    f"Smoke Test: {test_name}", False, f"Request failed: {str(e)}"
                )
                all_passed = False
    ...
    def run_all_checks(self) -> bool:
        ...
        for check in checks:
            try:
                ...
                self.log_result(
                    check.__name__, False, f"Check failed with exception: {str(e)}"
                )
                all_passed = False
