...
class CIPerformanceAnalyzer:
    ...
    def analyze_test_performance(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "average_duration": sum(d[0] for d in durations) / len(durations) if durations else 0,
    ...
    def analyze_docker_performance(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "estimated_layers": len([line for line in lines if line.strip() and not line.strip().startswith('#')])
            }
            
        except Exception as e:
            return {"error": f"Failed to analyze Docker: {str(e)}"}
    
    def generate_optimization_recommendations(self, test_data: Dict, coverage_data: Dict, docker_data: Dict) -> List[str]:
        ...
        if "coverage_percentage" in coverage_data:
            ...
            if coverage < 80:
                recommendations.append(f"📊 Test coverage is {coverage:.1f}% - consider adding more tests")
            elif coverage > 95:
        ...
        if "run_commands" in docker_data:
            ...
            if run_commands > 10:
                recommendations.append(f"🐳 Consider combining RUN commands (currently {run_commands})")
    ...
    def generate_performance_report(self) -> Dict:
        ...
        recommendations = self.generate_optimization_recommendations(test_data, coverage_data, docker_data)
    ...
    def save_report(self, report: Dict, output_file: str = "ci_performance_report.json"):
