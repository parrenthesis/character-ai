...
class EnvironmentValidator:
    ...
    def validate_single_var(self, rule: EnvVarRule) -> Dict[str, Any]:
        ...
        if rule.pattern and not re.match(rule.pattern, value):
            ...
            result["errors"].append(
                f"Value does not match required pattern: {rule.pattern}"
            )
        ...
        if rule.max_length and len(value) > rule.max_length:
            result["is_valid"] = False
            result["errors"].append(
                f"Value must be no more than {rule.max_length} characters long"
            )
    ...
    def validate_all_vars(self) -> Dict[str, Any]:
        ...
        return {
            "summary": {
                ...
                "completion_rate": (set_vars / total_vars * 100)
                if total_vars > 0
                else 0,
                "required_completion_rate": (required_set / required_vars * 100)
                if required_vars > 0
                else 0,
                "validity_rate": (valid_vars / total_vars * 100)
                if total_vars > 0
                else 0,
            },
            "results": results,
    ...
    def generate_report(self, validation_results: Dict[str, Any]) -> str:
        ...
        report.append(
            f"  Set Variables: {summary['set_variables']} ({summary['completion_rate']:.1f}%)"
        )
        report.append(
            f"  Valid Variables: {summary['valid_variables']} ({summary['validity_rate']:.1f}%)"
        )
        report.append(
            f"  Required Set: {summary['required_set']}/{summary['required_variables']} ({summary['required_completion_rate']:.1f}%)"
        )
        report.append("")
    ...
    def save_results(
        ...
        output_file: str = "env_validation_report.json",
    ):
