...
from character_ai.core.protocols import AudioData  # noqa: E402
from character_ai.production.real_time_engine import (  # noqa: E402
    RealTimeInteractionEngine,
)
from character_ai.production.toy_setup import ToySetup  # noqa: E402
...
async def demo_child_interaction(engine: RealTimeInteractionEngine):
    ...
    fake_audio = AudioData(
        data=b"fake_audio_data_from_child", sample_rate=16000, channels=1, format="wav"
    )
    ...
    if result.error:
        ...
        print(f'   "{result.text}"')
        print(
            f"   *plays audio response in {result.metadata.get('voice_style', 'default')} voice*"
        )
