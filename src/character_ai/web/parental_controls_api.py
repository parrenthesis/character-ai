...
from ..core.parental_controls import (
    ChildAgeGroup,
    ParentalControlManager,
    get_parental_control_manager,
)
...
def get_parental_control_manager_instance() -> ParentalControlManager:
    """Get the parental control manager instance."""
    global _parental_control_manager
    if _parental_control_manager is None:
        _parental_control_manager = get_parental_control_manager()
...
async def create_profile(
    ...
    try:
        manager = get_parental_control_manager_instance()
...
async def get_usage_report(
    ...
    try:
        ...
        # Get usage report
        report = manager.get_usage_report(child_id, days)
        if "error" in report:
...
async def get_safety_alerts(
    ...
    try:
        ...
        if child_id:
            alerts = [alert for alert in alerts if alert.user_id == child_id]
        if severity:
