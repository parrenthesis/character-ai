...
from ...observability import create_crash_reporter, get_logger
...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp):
        super().__init__(app)
        self.crash_reporter = create_crash_reporter()
    ...
    def _determine_component(self, path: str) -> str:
        ...
        if path.startswith("/api/v1/character/auth"):
            return "authentication"
        elif path.startswith("/api/v1/character/character"):
            return "character_management"
        elif path.startswith("/api/v1/character/memory"):
            return "session_memory"
        elif path.startswith("/api/v1/character/safety"):
            return "safety_filter"
        elif path.startswith("/api/v1/character/audio"):
            return "audio_processing"
        elif path.startswith("/api/v1/character/hardware"):
