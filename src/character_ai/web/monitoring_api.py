...
from ..monitoring.elk_integration import (
    ElasticsearchConfig,
    ELKIntegration,
    KibanaConfig,
    LogstashConfig,
...
from ..monitoring.prometheus_export import (
...
@monitoring_router.get("/elk/status")
async def get_elk_status() -> Dict[str, Any]:
    """Get ELK stack connection status."""
    try:
        ...
        elk = ELKIntegration(es_config, logstash_config, kibana_config)

        # Test Elasticsearch connection
        es_status = await elk.create_index_template()

        return {
            "elasticsearch": {
                "status": "connected" if es_status else "disconnected",
                "url": f"{es_config.scheme}://{es_config.host}:{es_config.port}",
            },
            "logstash": {"host": logstash_config.host, "port": logstash_config.port},
            "kibana": {
                "url": f"{kibana_config.scheme}://{kibana_config.host}:{kibana_config.port}"
            },
        }
    except Exception as e:
        logger.error(f"Failed to get ELK status: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def export_elk_configuration(
    ...
    try:
        ...
        if success:
            return {"message": f"ELK configuration exported to {output_dir}"}
        else:
            raise HTTPException(
                status_code=500, detail="Failed to export ELK configuration"
            )
    except Exception as e:
        logger.error(f"Failed to export ELK configuration: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def export_all_monitoring_configs(
    ...
    try:
        ...
        # Export ELK configuration
        elk_path = output_path / "elk"
        es_config = ElasticsearchConfig()
        logstash_config = LogstashConfig()
        kibana_config = KibanaConfig()
        elk = ELKIntegration(es_config, logstash_config, kibana_config)
        await elk.setup_elk_integration(elk_path)
...
- `elk/` - Elasticsearch, Logstash, and Kibana configuration files
...
2. Use the configuration files in `elk/` directory
3. Import the Kibana dashboard from `elk/kibana-dashboard.json`
...
- `GET /api/v1/monitoring/elk/status` - ELK stack status
