...
from .loaders import CharacterLoaderRegistry
from .types import CHARACTER_TEMPLATES, Character, CharacterTemplate
...
class CharacterService:
    ...
    def __init__(self) -> None:
        ...
        self.character_loader = CharacterLoaderRegistry()
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("Shutting down character manager...")
    ...
    async def _load_characters(self) -> None:
        ...
        try:
            ...
            for char_file in self.characters_dir.glob("**/profile.yaml"):
                ...
                # Pass the directory containing the profile.yaml file to the loader
                character_dir = char_file.parent
                character = await self._load_character_from_file(character_dir)
                if character:
                    # Use character ID as key if available, otherwise use name
                    character_key = character.metadata.get("id", character.name.lower())
                    self.characters[character_key] = character
                    logger.info(
                        f"Loaded character: {character.name} (key: {character_key})"
                    )
            ...
            for name, char in self.characters.items():
                logger.info(f"Character '{name}': metadata={char.metadata}")

        except Exception as e:
            logger.error(f"Error loading characters: {e}")

    async def _load_character_from_file(self, file_path: Path) -> Optional[Character]:
        """Load a character from a file using the character loader registry."""
        try:
            return self.character_loader.load_character(file_path)
        except Exception as e:
    ...
    async def create_custom_character(
        ...
        try:
            # Use existing AI generation functionality
            description = f"A {character_type.value} character named {name}"
            if custom_topics:
                description += f" interested in {', '.join(custom_topics)}"

            character = await self.generate_character_from_description(
                description, name
            )
            return character is not None
        except Exception as e:
            logger.error(f"Error creating custom character: {e}")
            return False
