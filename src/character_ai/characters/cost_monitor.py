...
class LLMCostMonitor:
    ...
    def __init__(self, cost_data_dir: Path = Path.cwd() / "data/cost_monitoring"):
        ...
        # Cost tracking files
        self.cost_entries_file = self.cost_data_dir / "cost_entries.json"
        self.monthly_summary_file = self.cost_data_dir / "monthly_summary.json"
    ...
    def _load_cost_entries(self) -> List[Dict[str, Any]]:
        """Load existing cost entries."""
        if self.cost_entries_file.exists():
            try:
                with open(self.cost_entries_file, "r") as f:
                    return json.load(f)  # type: ignore
            except Exception as e:
                logger.warning(f"Error loading cost entries: {e}")
        return []

    def _load_monthly_summary(self) -> Dict[str, Any]:
        """Load monthly cost summary."""
        if self.monthly_summary_file.exists():
            try:
                with open(self.monthly_summary_file, "r") as f:
                    return json.load(f)  # type: ignore
            except Exception as e:
                logger.warning(f"Error loading monthly summary: {e}")
        return {}
    ...
    def _save_cost_entries(self) -> None:
        ...
        try:
            with open(self.cost_entries_file, "w") as f:
                json.dump(self.cost_entries, f, indent=2)
        except Exception as e:
            logger.error(f"Error saving cost entries: {e}")

    def _save_monthly_summary(self) -> None:
        """Save monthly summary to file."""
        try:
            with open(self.monthly_summary_file, "w") as f:
                json.dump(self.monthly_summary, f, indent=2)
        except Exception as e:
    ...
    def export_cost_data(self, output_file: Optional[Path] = None) -> Path:
        ...
        if not self.cost_data_dir.exists():
