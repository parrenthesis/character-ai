...
class MultiLanguageAudioManager:
    """Unified multi-language audio processing manager."""

    def __init__(self, config: Config):
        self.config = config
        self.tts_manager: Optional[MultiLanguageTTSManager] = None
        self.stt_manager: Optional[MultiLanguageSTTManager] = None
        self.localization_manager: Any = get_localization_manager()
        self._initialized = False
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.tts_manager = MultiLanguageTTSManager(self.config)
            self.stt_manager = MultiLanguageSTTManager(self.config)
...
def get_multilingual_audio_manager(
    ...
    """Get the global multi-language audio manager instance."""
    ...
    if _multilingual_audio_manager is None:
        if config is None:
            ...
            config = Config()
        _multilingual_audio_manager = MultiLanguageAudioManager(config)
