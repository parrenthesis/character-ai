...
class CrashReporter:
    ...
    def _load_persisted_reports(self) -> None:
        ...
        data = JSONRepository.load_json(self.persistence_file, {})
        ...
        for report_data in reports_data:
            try:
                ...
                logger.warning(f"Failed to load crash report: {e}")

        logger.info(f"Loaded {len(reports_data)} persisted crash reports")
    ...
    def _save_reports_sync(self) -> None:
        ...
        success = JSONRepository.save_json(self.persistence_file, data)
        if success:
            logger.info(f"Saved {len(reports)} crash reports to persistence")
        else:
            logger.error("Failed to save crash reports")
...
def create_crash_reporter() -> CrashReporter:
    """Create a new crash reporter instance."""
...
def report_crash(
    ...
    return create_crash_reporter().report_crash(error, component, severity, context)
...
def report_error(
    ...
    return create_crash_reporter().report_error(
...
def get_health_status() -> HealthStatus:
    ...
    return create_crash_reporter().get_health_status()
