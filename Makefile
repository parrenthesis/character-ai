...
setup-ci:
	...
	poetry run pip install pytest pytest-asyncio pytest-cov pytest-mock pytest-benchmark black isort ruff mypy bandit detect-secrets safety types-PyYAML types-requests types-psutil --no-cache-dir --no-deps
	poetry run pip install pre-commit --no-cache-dir
