...
os.environ["PYTHONWARNINGS"] = "ignore::RuntimeWarning:sys:1,ignore::RuntimeWarning:unittest.mock,ignore::RuntimeWarning:tracemalloc"
...
warnings.filterwarnings("ignore", category=RuntimeWarning, message=".*_execute_mock_call.*")
warnings.filterwarnings("ignore", category=RuntimeWarning, message=".*add_relationship.*")
warnings.filterwarnings("ignore", category=RuntimeWarning, message=".*add_localization.*")
...
warnings.filterwarnings("ignore", category=RuntimeWarning, message=".*coroutine.*was never awaited")
warnings.filterwarnings("ignore", category=RuntimeWarning, message=".*AsyncMockMixin.*_execute_mock_call.*")
warnings.filterwarnings("ignore", category=RuntimeWarning, message=".*add_relationship.*_add_relationship.*")
warnings.filterwarnings("ignore", category=RuntimeWarning, message=".*add_localization.*_add_localization.*")
warnings.filterwarnings("ignore", category=RuntimeWarning, message=".*set_licensing.*_set_licensing.*")
...
@pytest.fixture(autouse=True)
def mock_log_aggregator():
    """Mock get_log_aggregator to prevent logs directory creation."""
    with patch('character_ai.core.log_aggregation.get_log_aggregator') as mock_get_log_aggregator:
...
@pytest.fixture(autouse=True)
def mock_external_deps(request):
    ...
    with patch.dict('sys.modules', {
            ...
            'TTS.tts.configs.vits_config': create_mock_module('TTS.tts.configs.vits_config'),
            ...
            'TTS.tts.utils.text.tokenizer': create_mock_module('TTS.tts.utils.text.tokenizer'),
            'TTS.tts.utils.text.phonemizers': create_mock_module('TTS.tts.utils.text.phonemizers'),
            'TTS.tts.utils.text.phonemizers.gruut_wrapper': create_mock_module('TTS.tts.utils.text.phonemizers.gruut_wrapper'),
            ...
            'transformers.AutoTokenizer': create_mock_module('transformers.AutoTokenizer'),
            'transformers.AutoModelForCausalLM': create_mock_module('transformers.AutoModelForCausalLM'),
            'safetensors': create_mock_module('safetensors'),
            'safetensors._safetensors_rust': create_mock_module('safetensors._safetensors_rust'),
